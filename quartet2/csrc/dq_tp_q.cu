...
{
    if (N % 128 != 0) throw std::invalid_argument("dequant_tp_quant: N must be divisible by 128");
    if (M % 128 != 0) throw std::invalid_argument("dequant_tp_quant: M must be divisible by 128");
    CHECK_POINTER(y);
    CHECK_POINTER(scales_fp8);
    CHECK_POINTER(scratch_scales);
    CHECK_POINTER(h);
    CHECK_POINTER(x);
    CHECK_POINTER(x_scales);
    CHECK_POINTER_NOT_NULL(global_scale_ptr);
    CHECK_POINTER_NOT_NULL(max_scale);
    CHECK_POINTER_NOT_NULL(x_global_scale);
