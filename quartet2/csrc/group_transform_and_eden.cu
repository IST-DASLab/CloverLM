...
{
    if (N % 128 != 0) throw std::invalid_argument("group_transform_128_eden: N must be divisible by 128");
    if (M % 128 != 0) throw std::invalid_argument("group_transform_128_eden: M must be divisible by 128");
    CHECK_POINTER(y);
    CHECK_POINTER(scales_fp8);
