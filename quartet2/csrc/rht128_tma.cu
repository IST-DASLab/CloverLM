...
void rht128_tma_launcher(nv_bfloat16* y, const nv_bfloat16* H, const nv_bfloat16* x, int M, int N) {
    if (N % 128 != 0) throw std::invalid_argument("rht128_tma: N must be divisible by 128");
    if (M % 128 != 0) throw std::invalid_argument("rht128_tma: M must be divisible by 128");
    CHECK_POINTER(y);
    CHECK_POINTER(H);
    CHECK_POINTER(x);
