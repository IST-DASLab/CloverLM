...
void rht128_ws_launcher(nv_bfloat16* y, const nv_bfloat16* H, const nv_bfloat16* x, int M, int N) {
    if ((M * N) % 128 != 0) throw std::invalid_argument("rht128_ws_launcher: M * N must be divisible by 128");
    CHECK_POINTER(y);
    CHECK_POINTER(H);
    CHECK_POINTER(x);
...
void transform_rht128_ws(nv_bfloat16* y, const nv_bfloat16* H, const nv_bfloat16* x, int M, int N, bool transpose) {
    if (transpose) throw std::invalid_argument("transpose not supported for RHT128");
