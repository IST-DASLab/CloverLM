...
void eden_fp4(__nv_fp4x4_e2m1* y_ptr, __nv_fp8_e4m3* scale_ptr, float* global_scale_ptr, const nv_bfloat16* x_ptr, const float* amax_ptr, float scale_override, long seed, long rows, long cols) {
    if (cols % 128 != 0) throw std::invalid_argument("eden_fp4: cols must be divisible by 128");
    if (rows % 128 != 0) throw std::invalid_argument("eden_fp4: rows must be divisible by 128");
    CHECK_POINTER(y_ptr);
    CHECK_POINTER(x_ptr);
