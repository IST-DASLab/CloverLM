...
inline void check_pointer_not_null(const void* ptr, const char* name, const char* file, int line) {
    if (ptr == nullptr) {
        const std::string msg = std::string("Pointer `") + name + "` is null in " + file + ":" + std::to_string(line);
        throw std::invalid_argument(msg);
    }
}


inline void check_pointer_alignment(const void* ptr, const char* name, const char* file, int line) {
    const auto ptr_as_int = reinterpret_cast<std::uintptr_t>(ptr);
    // require 16-byte alignment for all non-scalar inputs, so kernels
    // can be vectorized and use TMA
    if (ptr_as_int % 16 != 0) {
        const std::string msg = std::string("Pointer `") + name + "` at " + std::to_string(ptr_as_int) +
            " is not aligned to 16 bytes in " + file + ":" + std::to_string(line);
        throw std::invalid_argument(msg);
    }
}

inline void check_pointer_impl(const void* ptr, const char* name, const char* file, int line) {
    check_pointer_not_null(ptr, name, file, line);
    check_pointer_alignment(ptr, name, file, line);
}

#define CHECK_POINTER_NOT_NULL(ptr) check_pointer_not_null((ptr), #ptr, __FILE__, __LINE__)
#define CHECK_POINTER(ptr) check_pointer_impl((ptr), #ptr, __FILE__, __LINE__)
